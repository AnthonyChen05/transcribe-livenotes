...
1. Click **● Record** and allow microphone access.
2. Talk — a red dot pulses while listening; each pause ends an utterance and text appears within a second or two (a *transcribing…* badge shows while whisper works).
3. On the right, edit markdown notes freely. **Edit/Preview** toggles rendering. Drag the divider between the panes to resize them (double-click resets it). **◱ Hide** hides your editor so the pane shows only the AI's Live notes (a transcript + AI notes view); **◱ Show** brings the editor back. Both layout choices are remembered across reloads.
4. Toolbar (streams output straight into your notes at the cursor):
...
5. **Auto notes** (footer toggle, on by default): every ~20 s, if ≥ 250 new characters were transcribed, Ollama appends the new key points to the *Live notes* panel above the editor — an animated arrow between the panes marks while it runs. The bullets live in their own file (`data/autonotes.md`); your notes document is never modified by the AI, the UI just shows the two together. The panel grows incrementally — earlier points are kept, never rewritten — and displays the single latest update time. **↻ Update now** processes the material accumulated so far immediately. **⟳⟳ Rebuild all** discards the current bullets and regenerates the Live notes from the *entire* transcript — useful after clearing the notes or when the accumulated set has drifted; the old bullets stay until the new set is ready, so a failed rebuild never blanks the panel.
6. Notes autosave to `data/notes.md` (700 ms after you stop typing, or **Ctrl+S** immediately). The transcript persists to `data/transcript.md`, and a plain-text copy (no timestamps) is always kept up to date at `data/latest.txt` — handy for other tools to read mid-session. The transcript pane's **⤓ Export** button downloads the timestamped transcript as a `.txt` file.
7. **Profiles** (topbar) describe the event you're recording: topic / scenario, speaker accents, and a style guide. The active profile is injected into every AI prompt — summaries, action items, polish, and the auto-notes job — so the output matches the context (e.g. accent notes tell the model to interpret likely mishearings). Create and edit profiles via **Profiles…**; they're stored in `data/profiles.json`.

## Configuration

All settings live in `data/config.json` (model choice, auto-notes toggle) and are also editable from the UI. Environment overrides:
...
│   └── config.json       # model + auto-notes + active profile settings
